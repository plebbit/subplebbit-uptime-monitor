...
const {generatePrivateKey, getPublicKeyFromPrivateKey, getPlebbitAddressFromPrivateKey} = require('./utils/crypto')
...
const getChallenge = async (monitor) => {
  ...
  // encrypt publication
  const encryptedPublication = await encryptEd25519AesGcm(JSON.stringify(comment), pubsubMessageSigner.privateKey, monitor.subplebbit.signature.publicKey)
  ...
  const challengeRequestPubsubMessageSignedPropertyNames = ['type', 'timestamp', 'challengeRequestId', 'encryptedPublication', 'acceptedChallengeTypes']
  ...
  // TODO: plebbit-js bug that causes subplebbit.pubsubTopic to be incorrect
...
const publishPubsubMessage = async (pubsubTopic, messageObject, logInfo) => {
  ...
  messageReceivedPromise = new Promise((resolve, reject) => {
    onMessageReceived = async (rawMessageReceived) => {
      ...
      if (messageObject.type === 'CHALLENGEREQUEST' || messageObject.type === 'CHALLENGEANSWER') {
        if (messageReceivedObject.type === 'CHALLENGE' || messageReceivedObject.type === 'CHALLENGEVERIFICATION') {
          ...
          console.log({messageReceivedObject})
          pubsubIpfsClient.pubsub.unsubscribe(pubsubTopic).catch(console.log)
          console.log('unsub')
          received = true
