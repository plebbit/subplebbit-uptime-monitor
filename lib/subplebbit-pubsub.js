...
import {fetchJson, pubsubTopicToDhtKey, getPlebbitAddressFromPublicKey, getTimeAgo} from './utils.js'
...
const fetchPubsubDhtPeers = async (subplebbit, pubsubTopic) => {
  ...
  for (const delegatedRoutingUrl of config.delegatedRoutingUrls) {
    try {
      const jsonResponse = await fetchJson(`${delegatedRoutingUrl}/routing/v1/providers/${pubsubTopicDhtKey}`)
      let pubsubDhtPeers = jsonResponse?.Providers
      // no providers gives null, replace to empty array
      if (pubsubDhtPeers === null) {
        pubsubDhtPeers = []
      }
      if (!Array.isArray(pubsubDhtPeers)) {
        error = Error(`failed fetching pubsub dht peers for '${subplebbit.address}' from url '${delegatedRoutingUrl}' got response '${JSON.stringify(jsonResponse).substring(0, 300)}'`)
        continue
      }
